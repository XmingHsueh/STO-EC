...
algorithm_list = [4 3;5 5;5 7]; % three integration-based S-ESTO algorithms
    ...
    for n = 1:num_problems
        ...
        for r = 1:runs
            ...
            fprintf(['Algorithm: ','S',num2str(algorithm_list(a,1)),'+A',...
                num2str(algorithm_list(a,2)),', STOP-',num2str(n),', run: ',num2str(r),'\n']);
            waitbar(count/runs_total,h,sprintf('Optimization in progress: %.2f%%',...
                count/runs_total*100));
        end
        % save the results
        save(['.\experimental studies\results-rq5\',task_families{specifications(n,1)},'-T',...
            ...
            '+A-',opts_sesto.adaptations{algorithm_list(a,2)},'.mat'],'results_opt');
    end
end
close(h);