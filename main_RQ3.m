...
% adaptation models in the context of S-ESTO.
%
% ------------
% Reference:
% ------------
...
algorithm_list = [zeros(length(opts_sesto.adaptations),1),transpose(1:length(opts_sesto.adaptations))]; % similarty-driven S-ESTOs
            ...
            for s = 1:length(source_generations)
                for xi = xis
                    for d = ds
                        ...
                        % save the results
                        save(['.\experimental studies\results-rq3\',problem_families{p},'-',transfer_scenarios{t},'-',source_generations{s},'-x',num2str(xi),...
