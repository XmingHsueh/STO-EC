...
% Description:
% ------------
% This file is the entry point of experimentally comparing various transferability
% metrics for prioritizing the transferability in what to transfer.
%
% ------------
% Reference:
% ------------
...
folder_problems = '.\benchmarks-ea-k1000'; % the folder that stores the test problems
...
opts_sesto.transfer_mode = 'What'; % 'What', 'When', 'How'
...
algorithm_list = 1:length(opts_sesto.metrics);
...
for n = 1:num_problems
    ...
    for r = 1:runs
        ...
        for m = 1:length(algorithm_list)
            ...
            [solutions,objs] = transfer_evolutionary_search(problem,popsize,FEsMax,...
                optimizer,opts_sesto);
            solutions_all(:,m) = solutions;
            objs_all(:,m) = objs;
            count = count+1;
            fprintf(['Algorithm: What-',opts_sesto.metrics{algorithm_list(m)},', STOP-',num2str(n),', run: ',num2str(r),'\n']);
    ...
    % save the results
    save(['.\experimental studies\results-what-opt-k1000\',task_families{specifications(n,1)},'-T',...
        transfer_scenarios{specifications(n,2)},'-S',similarity_distributions{specifications(n,3)},...
        '-d',num2str(specifications(n,4)),'-k',num2str(specifications(n,5)),'-what.mat'],'results_opt');
end
close(h);
