...
function [solutions,objs,extras] = tea_search(problem,popsize,FEsMax,optimizer,paras)
    ...
    switch(transfer_mode)
        case 'What'
            if mod(gen,gen_trans) == 0
                ...
                solution_transfer = transferability_measurement(population_parent,obj_parent,lb,ub,gen,knowledge_base,metric);
        ...
        case 'When'
            ...
            for i = 1:no_trans
                ...
                [solution_transfer,state] = transferability_evaluation(population_parent,obj_parent,lb,ub,gen,knowledge_base,idx_source,evaluator);
                extras.states(gen,i) = state;
                idx = sequence_trans(i);
                extras.gains(gen,i) = (-1)*(fun(population_child(idx,:))<=fun(solution_transfer))+1*(fun(population_child(idx,:))>fun(solution_transfer));
                if state == 1
        ...
        case 'How'
            if mod(gen,gen_trans) == 0
                ...
                solution_transfer = transferability_improvement(population_parent,obj_parent,lb,ub,gen,knowledge_base(idx_source),solution_unadapted_normalized,adaptation);
                if ~isempty(solution_transfer)
                    idx = randi(popsize);
                    extras.qualities(gen,:) = [fun(lb+solution_unadapted_normalized.*(ub-lb)), fun(solution_transfer), fun(population_child(idx,:))];
                    population_child(idx,:) = solution_transfer;
                end
            end
    end
